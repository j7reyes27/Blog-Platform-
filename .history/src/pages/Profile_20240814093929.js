...
const Profile = () => {
    ...
    useEffect(() => {
        ...
        if(storedUser && storedUser.username === username) {
            ...
            setValue('avatar', storedUser.avatar); // Fix the avatar key (from 'Avatar' to 'avatar')
        }
    }, [username, setValue]);
    ...
    useEffect(() => {
        const fetchArticles = async () => {
            if (user) {
                try {
                    const response = await fetch(`https://api.realworld.io/api/articles?author=${user.username}&limit=100`);
                    const data = await response.json();
                    console.log('Fetched articles:', data); // Log the entire data object
    ...
    if (!user) return <div>Loading my brotha...</div>;
