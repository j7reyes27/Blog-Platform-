...
const Register = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response = await axios.post('https://realworld.habsidev.com/api/users', {
        user: userData,
      });
  ...
  return (
    <div className='register-container'>
      <form onSubmit={handleSubmit(onSubmit)} className='register-form'>
        ...
        <input type="text" placeholder="some-username" {...register('username', { required: 'Username is required', minLength: { value: 3, message: 'Minimum length is 3' } })} />
