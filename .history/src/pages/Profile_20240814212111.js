            ...
            if (user) {
                try {
                    const response = await fetch(`https://api.realworld.io/api/articles?author=${username}`, {
                        headers: {
                            'Authorization': `Token ${localStorage.getItem('your_token_key')}`,
                            'Origin': 'https://api.realworld.io'
                        }
                        
                    });
                    if (response.status === 401) {
                        console.error('Unauthorized: Check if the token is required for this request.');
                        // You could display a message to the user or handle this case appropriately
                    }
