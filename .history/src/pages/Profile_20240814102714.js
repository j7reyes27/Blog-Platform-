...
const Profile = () => {
    ...
    useEffect(() => {
        const fetchArticles = async () => {
            if (user) {
                try {
                    // Directly fetch the article with the known slug
                    const response = await fetch(`https://api.realworld.io/api/articles/I-will-rule-the-world!-48842`);
                    const data = await response.json();
                    console.log('Fetched article:', data.article); // Log the fetched article
                    setArticles([data.article]); // Store the article in an array to match the structure
                } catch (err) {
