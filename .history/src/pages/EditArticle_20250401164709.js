...
const EditArticle = () => {
  ...
  useEffect(() => {
    const fetchArticle = async () => {
      ...
      try {
        const response = await axios.get(`https://realworld.habsidev.com/api/articles/${slug}`, {
          headers: {
        ...
        <div className="tags-input">
          {tags.map((tag, index) => (
            <div key={index} className="tag">
              <span>{tag}</span>
              <button type="button" onClick={() => handleTagDelete(index)} className="delete-tag">
                Delete
              </button>
            </div>
          ))}
          <div className="tag-input-container">
            ...
            />
            <button type="button" onClick={handleTagAdd} className="add-tag">
              Add tag
            </button>
          </div>
        </div>
